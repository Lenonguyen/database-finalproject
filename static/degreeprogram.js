...
var request = new XMLHttpRequest();
    request.open("GET", "http://localhost:8000/degreeprograms/", false);
    ...
    request.send();
arrayDegree = JSON.parse(request.responseText);
var display = "<table><tr><th>Code</th><th>Name </th></tr>";
for(var i=0;i<arrayDegree.length;i++){
    display+="<tr><td>" + arrayDegree[i].code + "</td><td>" + arrayDegree[i].name + "</td></tr>";
}
display+="</table>";
...
function postDegree(){
 ...
 request.open("POST", "http://localhost:8000/degreeprograms/", false);
 request.setRequestHeader("Authorization", "Basic " + btoa("admin:password123"));
 request.setRequestHeader("X-CSRFToken", readCookie("csrftoken"));
 request.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
